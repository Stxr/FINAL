...
//		return 0;
	}else
	{
	 ...
	 RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB|RCC_APB2Periph_GPIOG, ENABLE);	 //使能PB,PF端口时钟
		...
		GPIO_InitStructure.GPIO_Pin = GPIO_Pin_14;				 // PB2端口配置  MISO:PB2-->PB14   
	 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU; 		 //上拉输入
	 	GPIO_Init(GPIOB, &GPIO_InitStructure);//B2上拉输入
	 	GPIO_SetBits(GPIOB,GPIO_Pin_14);//上拉		
		
		GPIO_InitStructure.GPIO_Pin = GPIO_Pin_15;				 // F9，PF11端口配置  MOSI:PF9-->PB15
	 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
	 	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	 	GPIO_Init(GPIOB, &GPIO_InitStructure);//PF9,PF11推挽输出
	 	GPIO_SetBits(GPIOB, GPIO_Pin_15);//上拉
		
		GPIO_InitStructure.GPIO_Pin = GPIO_Pin_11;				 // F9，PF11端口配置  CS:PF11-->PG11
	 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
	 	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	 	GPIO_Init(GPIOG, &GPIO_InitStructure);//PF9,PF11推挽输出
	 	GPIO_SetBits(GPIOG, GPIO_Pin_11);//上拉
		
		GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8;				 // PF10端口配置  TPEN:PF10-->PG8
	 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU; 		 //上拉输入
